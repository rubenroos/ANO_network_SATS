...
For building network models, we want a data file that contains all the "kartleggingsenheter" across the data set, their species, and the cumulative abundance of all individual species.

```{r Create data set for network analyses}

#calculate species cover per naturype per flate (sum eller average per naturtype per flate?)
...
ANO.type <- ANO.fjell %>% 
  ...
  summarise(nature_type = names(which.max(table(kartleggingsenhet_1m2))))

# Create data set that sums all species cover per ANO-flate
ANO.cover <- ANO.fjell %>% 
   select(ano_flate_id, ano_punkt_id, kartleggingsenhet_1m2, Species, art_dekning) %>% group_by(ano_flate_id, Species) %>% 
  summarize(cover_sum = sum(art_dekning))

# Join these two data frames and convert to wide format
ANO.netw <- ANO.cover %>% 
  left_join(ANO.type, by = "ano_flate_id") %>% 
  spread(Species, cover_sum, fill = 0) %>% 
  rename()
  
#we can remove the first column with rownames and create a matrix
ANO_netw_ <- ANO.netw[,-1]
ANO_netw <- as.matrix(ANO_netw_)
colnames(ANO.netw) <- colnames(ANO_netw_)
