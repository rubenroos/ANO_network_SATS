...
ANO.fjell <- ANO.fjell %>% 
  filter(!(art_dekning == 0)) %>% #We should remove the plots where only one species without cover was registered (it is obligatory to list at least one species in the data sampling protocol, even when no species are present)
...
ANO.fjell %>%
  ...
  count() %>% 
  arrange(desc(n)) #This produced duplicates before, but the directorate for enviroment has solved this issue in the data. 

#The code below is to remove any duplicates, which is no longer necessary. ¨
