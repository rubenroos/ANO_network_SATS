...
#Remove the Flate/sites with duplicated data entirely. To do this, we first make a list, and then filter the data on that list. 
...
ANO.cover <- ANO.cover %>% 
  pivot_wider(id_cols = c(Site), names_from = Species, values_from = Species_cover, values_fill = 0)
...
ANO.cover.sum <- ANO.cover.sum %>% pivot_wider(names_from = Species, values_from = Species_cover_summed, values_fill = 0)
