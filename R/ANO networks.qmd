...
df.matrix <- as.matrix(df[4:364])
rownames(df.matrix)<- df$Plot

#We can use package _bipartite_ to plot it in two different ways:

#prepare legend
legend <- unique(Nature.types[,c("Nature_type", "Color")])
par(xpd = T) #allow plotting outside the plot
plotweb(df.matrix, col.low = as.character(Nature.types$Color))
...
visweb(df.matrix, prednames = T, preynames = T, labsize = 0.6)
