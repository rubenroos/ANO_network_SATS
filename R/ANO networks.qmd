...
ANO.fjell <- ANO.fjell %>% 
  group_by(ano_punkt_id) %>% 
  mutate(dekning_sum = sum(art_dekning), 
            rel_dekning = art_dekning/dekning_sum)
...
For building network models, we want a data file that contains all the "kartleggingsenheter" across the data set, their species, and the cumulative abundance of all ndividual species.
